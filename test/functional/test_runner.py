...
`test/functional/test_framework/test_framework.py:BTQTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_btq_cli.py --legacy-wallet',
    'interface_btq_cli.py --descriptors',
...
def main():
    ...
    enable_btqd = config["components"].getboolean("ENABLE_BTQD")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if btqd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "btqd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a btqd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/btq/btq/pull/27561.
...
class RPCCoverage():
    ...
    commands per `btq-cli help` (`rpc_interface.txt`).
