#!/usr/bin/env python3
# Copyright (c) 2014-2022 The BTQ Core developers
...
class RawTransactionsTest(BTQTestFramework):
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid btq address", self.nodes[2].fundrawtransaction, rawtx, changeAddress='foobar')
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sat/vB and feeRate BTQ/kvB)")
        node = self.nodes[3]
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (BTQ/kvB)",
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 BTQ
        ...
        # Fund wallet with 2 outputs, 5 BTQ each.
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 BTQ each (20 BTQ total)
        ...
        # Attempt to send 29 BTQ from a wallet that only has 20 BTQ. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 BTQ payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 BTQ.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 BTQ)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 BTQ.
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 BTQ (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 BTQ/kvB in this test.
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 BTQ target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 BTQ) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 BTQ covered (need to fund 0.9 BTQ more).
